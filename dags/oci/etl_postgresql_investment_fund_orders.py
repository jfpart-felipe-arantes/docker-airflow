...
with DAG(
    "etl_postgresql_investment_fund_orders",
    default_args=default_args,
    description="Load investment_fund_orders in bucket OCI",
    schedule_interval="0 6,10,14,18 * * 1-5",
    start_date=pendulum.datetime(2023, 1, 1, tz=local_tz),
    catchup=False,
    tags=["bucket", "postgres", "investment_fund_orders"],
    ...
    def extract_data_postgre():
        ...
        query = """ SELECT 
                    * FROM public.investment_fund_orders;"""
    ...
    def load_csv(df):
        ...
        df_parquet.to_parquet("oci://bkt-prod-veritas-bronze@grilxlpa4rlr/POSTGRESQL/INVESTMENT_FUND_ORDERS/investment_fund_orders.parquet.gzip", compression='gzip',storage_options={"config": config},)
