...
with DAG(
    "etl_positions_wallet",
    default_args=default_args,
    description="Load into MongoDB postions from PMS",
    ...
    def extract_data():
        ...
        query = f"""
                    ...
                    Where PS.qt_Cotas > 0
                    Group By PE.cd_CPFCNPJ, DEF.id_Fundo, PEP.ds_Nome, DEF.nr_DiasConvRes, DEF.tp_DiasConvRes, CC.Pr_NotaRisco, DEF.fl_FechadoAplicacao, DEF.fl_FechadoNovaAplicacao 
        """ 
            ...
            assetRentabilityDescription, amount, earnings, netValue, \
                type_, Owner, iof, irrf, quoteDeadline, quoteDeadlineDaysType, dueDate, dailyLiquidity, \
                    liquidityDescription, applicationDate, gracePeriod, graceDate, productId, \
                        suitabilityProfile, closedApplication, closedNewApplication, position_id, update_at in data:
    ...
    def extract_prev():
        collection = connect_mongo(
            Variable.get('MONGO_URL_PREV'),
            Variable.get('MONGO_DB_PREV'),
            Variable.get('MONGO_COL_PREV')
        )        
        clients_prev = collection.find({})
        return clients_prev


    def ingest_prev(clients_prev, mongo_engine):
        collection = mongo_engine
        novo_asset = {
            'idProduct': 0,
            'assetDescription': '',
            'assetIssuer': '',
            'assetRentabilityDescription': '',
            'amount': 0,
            'earnings': 0,
            'netValue': 0,
            'Owner': 'Original',
            ...
            'suitabilityProfile': '', 
            'closedApplication': '', 
            'closedNewApplication': '',
            'position_id': ''
        }
        for document in clients_prev:
            cpf_cnpj = ('00000000000' + str(document.get('NUM_CPF_CNPJ')))[-11:]
            if collection.find_one({'document': cpf_cnpj}):
                if collection.find_one({'$and':[{'document': cpf_cnpj},{'assets.type': 'PrivatePension'}]}):
                    info(f'O documento já possui previdência em seus assets, {cpf_cnpj[:5]}******')
                else:
                    results = collection.update_one(
                        {'document': cpf_cnpj},
                        {'$push': {'assets': novo_asset}}
                    )
                    info(f'Foi inserido um asset de previdencia, {results.modified_count}')
            else:
                novo_documento = {
                    'document': cpf_cnpj,
                    'assets': []
                }
                novo_documento['assets'].append(novo_asset)
                collection.insert_one(novo_documento)
                info(f'Foi inserido um novo documento {cpf_cnpj[:5]}******')  
    ...
    def main(**context):
        ...
        try:
            ...
            info("Initializing extraction Prev")
            clients_prev = extract_prev()
            info("Initializing ingest Prev")
            ingest_prev(clients_prev, mongo_engine)
            mongo_engine = connect_mongo(
                Variable.get("LIGA_MONGO_URL_WALLET_POSITIONS"),
                Variable.get("LIGA_MONGO_DB_WALLET_POSITIONS"),
                Variable.get("LIGA_MONGO_COL_WALLET_POSITIONS"),
            )
            info("Initializing loading env Liga")
            load_data(data_transformed, mongo_engine)
            ingest_prev(clients_prev, mongo_engine)
    ...
    :alert: *ERROR - WALLET - ETL Positions* :alert:
    *Dag*: {dag}
    *Error*: {error}
    *Execution Time*: {exec_date}
                """.format(
                        dag="etl_positions_wallet",
                        error=error,
            ...
            slack_msg = """
    :white_check_mark: *SUCCESS - WALLET - ETL POSITIONS*
    *Dag*: {dag}
    *Execution Time*: {exec_date}
                """.format(
                        dag="etl_positions_wallet",
