...
with DAG(
    "etl_positions_wallet_investpro",
    default_args=default_args,
    description="Load into MongoDB Picpay Invest postions from Bankpro and Sirsan",
    schedule_interval= '0/5 3-18 * * 1-5',
    start_date=datetime(2021, 1, 1),
    catchup=False,
    tags=["Positions", "Wallet", "investpro"],
    max_active_runs=1,
) as dag:
    ...
    def extract_data():
        ...
        query_bankpro = """
             SELECT DISTINCT 
                ...
                isnull(convert(float,a.ValorAtualDisponivel),0) AS netValue,
                'FixedIncome' as type,
                'PicPay' as Owner,
                ...
                END AS graceDate,
                ISNULL(a.idproduto, 0) AS productId,
                ISNULL(a.idproduto, 0) AS idProduct,
        ...
        for document, id, assetDescription, assetIssuer, \
            assetRentabilityDescription, amount, earnings, netValue, \
                type_, Owner, iof, irrf, quoteDeadline, quoteDeadlineDaysType, dueDate, dailyLiquidity, \
                    liquidityDescription, applicationDate, gracePeriod, graceDate, productId, idProduct, \
                        offerId, suitabilityProfile, closedApplication, closedNewApplication, position_id, \
                            liquidationDeadline, liquidationDeadlineDaysType,update_at in data:
    ...
    :alert: *ERROR - WALLET INVESTPRO - ETL POSITIONS* :alert:
    *Dag*: {dag}
    *Error*: {error}
    *Execution Time*: {exec_date}
                """.format(
                        dag="etl_positions_wallet_investpro",
                        error=error,
            ...
            slack_msg = """
    :white_check_mark: *SUCCESS - WALLET INVESTPRO - ETL POSITIONS*
    *Dag*: {dag}
    *Execution Time*: {exec_date}
                """.format(
                        dag="etl_positions_wallet_investpro",
    ...
    b3_is_open >> continue_task >> lambda_task >> end_task