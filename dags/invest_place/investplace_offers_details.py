...
with DAG(
    "investplace_offers_details",
    default_args=default_args,
    description="Load offers_details in mysql of PIT",
    schedule_interval= '0 10 * * 1-5',
    start_date=datetime(2021, 1, 1),
    catchup=False,
    tags=["offers_details", "investplace", "pit"],
    max_active_runs=1,
) as dag:

    dag.doc_md = __doc__

    #tabela auxiliar para carga da details
    def bridge_offer():
        df = pd.read_sql(
                f"""        
            SELECT 	case
    ...
    def insert_data():
        ...
        query = """select c.offer_id,
        offer_metadata_id as offer_metadata_id,
        CASE 
            when a.offer_metadata_id = 78 then b.distribution_fee
            when a.offer_metadata_id = 81 then b.fgc_guarantee
            when a.offer_metadata_id = 83 then b.issuer
            when a.offer_metadata_id = 84 then b.custodian_name
            when a.offer_metadata_id = 85 then b.broker_name
            when a.offer_metadata_id = 86 then b.risk_rating
            when a.offer_metadata_id = 87 then b.description_liquidity
            when a.offer_metadata_id = 99 then b.ir_free 
        END as value,
        now() AS created_at,
        now() AS updated_at,
        214 as updated_by
        from offers.offers_metadata a
        inner join offers_bridge b
        on (a.product_type_id = b.product_type_id
        or a.product_type_id = 4)
        inner join offers.offers c
        on c.external_code = b.offer_id
        where offer_metadata_id in (78,81,83,84,85,86,87,99)"""
        print("Extract complete")
    ...
    def send_slack_message_func(error, type):
        ...
        :alert: *ERROR - INVESTPLACE OFFERS_DETAILS* :alert:
        *Dag*: {dag}
        *Error*: {error}
        *Execution Time*: {exec_date}
                    """.format(
                            dag="investplace_offers_details",
                            error=error,
                ...
                slack_msg = """
        :white_check_mark: *SUCCESS - INVESTPLACE OFFERS_DETAILS*
        *Dag*: {dag}
        *Execution Time*: {exec_date}
                    """.format(
                            dag="investplace_offers_details",
