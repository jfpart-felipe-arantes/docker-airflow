...
with DAG(
    "etl_open_finance_fixed_income",
    default_args=default_args,
    description="Load fixed income from open finance",
    schedule_interval="0 4 * * 1-5",
    start_date=datetime(2021, 1, 1),
    catchup=False,
    tags=["open_finance", "Fixed_Income"],
) as dag:
    dag.doc_md = __doc__
    ...
    def investments_application(cnxn, **kwargs):
        logging.info("Start extraction SCMLionXOpen.dbo.vw_openfinance_rendafixa_investments_application")

        source_table = "SCMLionXOpen.dbo.vw_openfinance_rendafixa_investments_application"

        truncate_query = "truncate table SCMLionXOpen.dbo.tb_openfinance_rendafixa_investments_application"

        destination_table = "SCMLionXOpen.dbo.tb_openfinance_rendafixa_investments_application"

        template_connection(cnxn, source_table, truncate_query, destination_table)  

        logging.info("End of insertion in tb_openfinance_rendafixa_investments_application")

    def balances(cnxn, **kwargs):
        
        logging.info("Start extraction SCMLionXOpen.dbo.vw_openfinance_rendafixa_balances")

        source_table = (
            "SCMLionXOpen.dbo.vw_openfinance_rendafixa_balances"
        )

        truncate_query = (
            "truncate table SCMLionXOpen.dbo.tb_openfinance_rendafixa_balances"
        )

        destination_table = (
            "SCMLionXOpen.dbo.tb_openfinance_rendafixa_balances"
        )

        template_connection(cnxn, source_table, truncate_query, destination_table)

        logging.info("End of insertion in tb_openfinance_rendafixa_balances")

    def investments(cnxn, **kwargs):

        logging.info("Start extraction SCMLionXOpen.dbo.vw_openfinance_rendafixa_investments")

        source_table = "SCMLionXOpen.dbo.vw_openfinance_rendafixa_investments"

        truncate_query = "truncate table SCMLionXOpen.dbo.tb_openfinance_rendafixa_investments"

        destination_table = (
            "SCMLionXOpen.dbo.tb_openfinance_rendafixa_investments"
        )

        template_connection(cnxn, source_table, truncate_query, destination_table)

        logging.info("End of insertion in tb_openfinance_rendafixa_investments")


    def investments_investmentid(cnxn, **kwargs):
        logging.info("Start extraction SCMLionXOpen.dbo.vw_openfinance_rendafixa_investments_investmentid")

        source_table = "SCMLionXOpen.dbo.vw_openfinance_rendafixa_investments_investmentid"

        truncate_query = "truncate table SCMLionXOpen.dbo.tb_openfinance_rendafixa_investments_investmentid"

        destination_table = "SCMLionXOpen.dbo.tb_openfinance_rendafixa_investments_investmentid"

        template_connection(cnxn, source_table, truncate_query, destination_table)  

        logging.info("End of insertion in tb_openfinance_rendafixa_investments_investmentid")

    def transactions():

        cnxn = create_connection()
        logging.info("Start extraction SCMLionXOpen.dbo.vw_openfinance_rendafixa_transactions")

        source_table = "SCMLionXOpen.dbo.vw_openfinance_rendafixa_transactions"

        truncate_query = "truncate table SCMLionXOpen.dbo.tb_openfinance_rendafixa_transactions"

        destination_table = (
            "SCMLionXOpen.dbo.tb_openfinance_rendafixa_transactions"
        )

        template_connection(cnxn, source_table, truncate_query, destination_table) 

        logging.info("End of insertion in tb_openfinance_rendafixa_transactions")

    def transactions_current():
        cnxn = create_connection()
        logging.info("Start extraction SCMLionXOpen.dbo.vw_openfinance_rendafixa_transactions_current")

        source_table = "SCMLionXOpen.dbo.vw_openfinance_rendafixa_transactions_current"

        truncate_query = "truncate table SCMLionXOpen.dbo.tb_openfinance_rendafixa_transactions_current"

        destination_table = (
            "SCMLionXOpen.dbo.tb_openfinance_rendafixa_transactions_current"
        )

        template_connection(cnxn, source_table, truncate_query, destination_table) 

        logging.info("End of insertion in tb_openfinance_rendafixa_transactions_current")

    def send_slack_message_func():

        slack_msg = """
<@U04V21FNP16>
:alert: *ERROR - OPEN FINANCE - ETL FIXED INCOME* :alert:
*Dag*: {dag}
*Error*: {error}
*Execution Time*: {exec_date}
            """.format(
                    dag="etl_open_finance_fixed_income",
